...
# classify_and_scan 的 overlays 自 v0.4.0 M2 起是必填关键字（防"漏传就当没开"的静默
# 分叉）。本文件钉的是五类判定/字段/排序，叠加层全关 = 恒等变换，期望值一个都不用改；
# 叠加层语义与三入口一致性在 tests/test_strategy_pipeline.py。
OFF = OverlaysCfg()
...
def test_stale_when_latest_bar_before_expected():
    ...
    assert classify_and_scan(bars, [ExplodingStrategy()], EXPECTED, 5e7, overlays=OFF) == ([], "stale")
...
def test_date_truncation_scans_as_of_expected_ignoring_later_bars():
    ...
    signals, skip = classify_and_scan(bars, [BUY], expected, 5e7, overlays=OFF)
    assert skip is None
...
def test_date_truncation_applies_before_warmup_count():
    ...
    assert classify_and_scan(bars, [BUY], expected, 5e7, overlays=OFF) == ([], "insufficient_history")
...
def test_insufficient_history_below_min_bars():
    ...
    assert classify_and_scan(bars, [ExplodingStrategy()], EXPECTED, 5e7, overlays=OFF) == (
        [], "insufficient_history")


def test_exactly_min_bars_passes_warmup():
    bars = scan_bars(n=MIN_BARS)
    signals, skip = classify_and_scan(bars, [HOLD], EXPECTED, 5e7, overlays=OFF)
...
def test_st_on_latest_bar_skipped():
    ...
    assert classify_and_scan(bars, [ExplodingStrategy()], EXPECTED, 5e7, overlays=OFF) == ([], "is_st")
...
def test_liquidity_at_threshold_passes():
    ...
    assert classify_and_scan(bars, [HOLD], EXPECTED, 5e7, overlays=OFF) == ([], None)
...
def test_only_new_buy_emits_signal():
    ...
    bars = scan_bars()
    signals, skip = classify_and_scan(bars, [BUY, HOLD, SELL], EXPECTED, 5e7, overlays=OFF)
    assert skip is None
...
def test_two_strategies_can_both_signal_same_symbol():
    bars = scan_bars()
    signals, skip = classify_and_scan(bars, [BUY, TailStub("buy2", [0, 1])], EXPECTED, 5e7, overlays=OFF)
    assert skip is None
    assert [s["strategy"] for s in signals] == ["buy", "buy2"]


def test_no_signal_returns_none_skip_and_empty_list():
    assert classify_and_scan(scan_bars(), [HOLD], EXPECTED, 5e7, overlays=OFF) == ([], None)
