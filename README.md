# quant_demo — A 股日线技术信号系统 v0.1.1

一个**纯学习用**的 A 股日线研究流水线：拉数据 → 算指标 → 策略出目标仓位 → 历史回测（含 A 股交易规则与成本）→ 绩效报告 → 收盘后的每日信号提示与全市场扫描（v0.1.1 新增）。目标是把这条链路跑通且**跑对**，不是产出能赚钱的策略；内置的双均线、唐奇安通道突破只是用来验证流水线的样品。
...
# 期望：221 passed, 3 deselected  （deselected 的 3 项是需要联网的 baostock 集成测试）
...
浏览器打开三个页面：**回测报告**（选历史某次回测看指标表/净值图/成交明细）、**个股 K 线**（选标的看买卖点标注）、**今日信号**（读 `output/signals/` 的清单与历史；页尾"全市场扫描"区块展示 `output/scan/` 最新一份扫描结果，标题带扫描日期——注意核对是不是今天的）。

面板是**纯只读**的：它只读 `output/` 与 `data/cache/`，不触发任何计算。没结果可看就是还没跑过 ①/②/③，页面会提示对应命令。
