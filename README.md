...
- **预估耗时**（2026-08-24 实测 50 只外推；逐票串行取数是 baostock 单连接的天花板）：冷缓存 50 只 163 秒、热缓存重跑同批 194 秒——**约 3.3–3.9 秒/只，缓存不省时间**（省的是不重拉 10 年历史，但每票仍要一次增量联网请求，网络往返是主项）。全量约 3200 只**单次预估约 3 小时**，请当成挂机任务。中断随时 Ctrl-C，重跑不会重拉已缓存的历史。日常建议 `--limit` 试跑确认环境正常再上全量，嫌慢等将来接入全市场快照源（接口已留插槽）。
...
- **逐票串行取数是 baostock 单连接的天花板**：单次全量约 3 小时，且实测缓存无法加速扫描本身（每票必有一次增量联网往返，见入口 ③ 的实测数据）。接口分层已留好快路径插槽（如 akshare 全市场快照），需要时替换"当日 bar 来源"即可，扫描逻辑不用动。
