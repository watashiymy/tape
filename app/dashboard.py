...
# 侧栏只留导航 + 一句免责声明（2026-09-05）。原先常驻的两条说明——"第一次用先看
# 使用说明""本面板可在本机启动任务，详情见控制台"——一条是导航在讲自己，一条是
# 控制台页头已经说了的话；使用者看了没有任何可做的事。
st.sidebar.caption("仅供学习研究，不构成投资建议。")
# 安全警告**只在适用时**出现：面板能执行本机命令，监听到局域网就等同于把远程命令
# 执行接口挂出去（设计 §5.3）。但 .streamlit/config.toml 已把默认绑到 127.0.0.1
# （从项目根目录启动时生效），天天对着一条不适用的警告等于没有警告——所以按
# 运行时真实的监听地址判断。措辞在 v0.2.1 M3 纠正过：streamlit 的**出厂**默认是
# 监听所有网卡（实测 *:8501），别再写"默认只监听本机"。
if not ui.is_loopback(st.get_option("server.address")):
    st.sidebar.warning("面板正监听非本机地址：同一网络里的人打开这个地址就能启动任务。"
                       "请从项目根目录启动，或加 `--server.address 127.0.0.1`；"
