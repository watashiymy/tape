...
**"漏接"有第二种形态**（M2 评审补）：走了 pipeline，但递进去的是就地构造的
`OverlaysCfg()`——配置里 atr_stop 写着开、实际按无叠加层跑，三个入口各自都输出一串
合法仓位，零告警。`overlays` 是必填参数管不住这个（填什么都是填了），所以断言必须
钉到**实参**上：三个 load_settings 的脚本里，叠加层实参只允许两种形态——
`settings.overlays`（配置里那份）或同名参数的转发 `overlays`。

再加一条行为侧证据：端到端真跑 `run_daily_signal.main()`（假交易日历 + 假行情，
其余全走真代码），同一段行情下配置 `atr_stop.enabled` 开 → CSV 里出现止损 SELL，
关 → 零信号。

**已证不可行的一条**：全市场扫描（`classify_and_scan`，只报 0→1 新 BUY）的结论
**对 atr_stop 恒等**，因此不可能用行为测试钉住它的接线。证明：入场当根 peak = 当根
收盘、回撤 0，故基础的 0→1 永远不会被压掉；反向，`pos[-1]=1` 要求末根未被封锁，
而解封只发生在 `prev_base == 0`（即 `base[-2]=0`），于是 `pos[-2]` 也是 0——
overlay 既不抹掉也不凭空造出新 BUY。这个入口只能靠上面的源码级实参断言；
**M3 的趋势过滤会打破这条恒等**（gate 挡住入场），届时补行为测试。
...
- 当日 `adj_close < peak − k × ATR(n)` → 目标仓位归 0（引擎既有的 shift 机制
  使其 T+1 开盘卖出，与所有既有信号同一时序，无未来函数）；判据是严格 `<`
  （回撤恰好等于阈值不触发）。
- **每次入场都重置 peak**：peak 从新入场那根的收盘起算，绝不沿用被止损那一轮的
  旧高点，也不是"全样本滚动最高"。这条决定每一笔的认输点，必须有专测，且专测的
  **二次入场价要落在旧 peak 之下**——否则 `max(旧peak, 新入场价) == 新入场价`，
  两种口径逐位一致，测试没有分辨力（M2 评审实测：一个跨轮沿用 peak 的变异体在
  1587 项里一条都不红）。
...
- **严格度不许只做一半**（M2 评审补）：认不出的叠加层**名**报错，叠加层**里面**
  认不出的参数名同样报错。`enabled` 漏个 d 写成 `enable`、`k` 打成 `kk` 都是常见
  手误，静默忽略的结果就是"配置写着开、实际关着"或"k 悄悄回到默认 3.0"，
  而回测照常 exit 0。同一口径也补给 `scan:` 段——它与 `overlays` 是配置里唯一两处
  `.get(键, 默认值)` 的段，也就是唯一两处键名拼错不会 KeyError 的段。
- 段的**形状**也校验：`atr_stop: 3.0`（把 k 直接写在叠加层名后面）此前抛
  `AttributeError: 'float' object has no attribute 'get'`，`overlays: 5` 抛
  `TypeError: 'int' object is not iterable`——响亮但报错离病因远。一律换成带段名与
  实际值的中文 ValueError。`overlays:`／`atr_stop:` 后面空着（YAML 读成 None）
  仍是合法写法 = 全默认，不许被形状校验顺手挡掉。

### 2.4 测试（本里程碑主要产出，手算对照）

- 手造序列：入场→连涨（peak 抬升）→回撤 2.9×ATR（不触发）→3.1×ATR（触发）→
  基础信号保持 1（**不得回补**）→基础信号 0→1（允许再入场）
- **二次入场价落在旧 peak 之下**的序列（peak 重置的唯一分辨力所在，见 §2.2）
- 暖机期不触发；截断重放无未来函数；overlay 关闭时 pipeline 结果 == 基础信号（恒等）
- 三入口一致性：同一 df 同一配置，三个入口算出的目标仓位逐位相同
- 三个脚本递的是 `settings.overlays`（源码级钉到实参）+ 端到端跑
  `run_daily_signal.main()` 验"配置开=报出止损 SELL，配置关=零信号"（见 §2.1）
- 配置严格性：叠加层内部拼错的键名、非映射形态的段（见 §2.3）
...
- **已知的具体脱节点**（M2 评审记账，M4 逐条清）：README「已知取舍」那条
  "无组合级风控与止损（都在 v0.2+ 的计划里）"已被 M2 推翻（`config/settings.yaml`
  默认 `atr_stop {enabled: true, n: 20, k: 3.0}`）；README/guide 通篇还没有 overlays
  段的说明；README 与 `guide.FACTS` 的 116.1%/48.9% 仍是无叠加层口径。
